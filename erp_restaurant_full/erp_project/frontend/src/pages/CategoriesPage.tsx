...
export default function CategoriesPage() {
  ...
  const [form, setForm] = useState({ name: '', nameAr: '', icon: '', description: '', sortOrder: '0' });
  ...
  const openEdit = (c: any) => { setEditing(c); setForm({ name: c.name, nameAr: c.nameAr, icon: c.icon || '', description: c.description || '', sortOrder: c.sortOrder.toString() }); setModal(true); };
  const openNew = () => { setEditing(null); setForm({ name: '', nameAr: '', icon: '', description: '', sortOrder: '0' }); setModal(true); };
  return (
    <div>
