...
- Split tender (part gift card / part cash / card) is now exposed in the POS UI: the cashier adds
  one or more payment lines (each with method + amount, gift-card tenders prompt for the card
  code), the panel shows Paid / Remaining / Change, and **Complete sale** is enabled only once the
  tenders cover the total. Each tender is posted as its own `POST /sales/orders/:id/payments`
  before completion.
- Added the missing `prisma/migrations/migration_lock.toml` (provider = postgresql) so
  `prisma migrate deploy` and drift checks work against a fresh checkout.
